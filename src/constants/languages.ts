 ...
 *
 * @remarks
 * **Responsible for:** The four supported languages; per-language UI strings;
 * one-line AI “respond only in X” prompts; and lesson opening greetings.
 *
 * **Talks to:**
 * - Imports: none (leaf constants module).
 * - Exports: `LanguageCode`, `Language`, `LANGUAGES`, `LANGUAGE_PROMPTS`, `UI_TEXT`,
 *   `getUIText`, `getGreeting`.
 * - Imported by: all `app/*` screens, `@/store/appStore`, `@/constants/subjects`,
 *   `@/services/aiService`.
 *
 * **Notes for new developers:**
 * - Add a new language by extending `LanguageCode`, `LANGUAGES`, `UI_TEXT`,
 *   `LANGUAGE_PROMPTS`, and greeting maps — then update `subjects` translations.
 * - `getUIText` has no fallback; an invalid code will throw at runtime.
 * - Brand name in UI is “Learnova” while the repo folder is `naija-learn`.
 */
